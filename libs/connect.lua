...
local base64, random, hmac, digest = ssl.base64, ssl.random, ssl.hmac, ssl.digest
local encodeJSON, decodeJSON = json.encode, json.decode
...
return function(options)
	...
	local function connectToRethinkdb()
		...
		local salt = base64(auth.s, false)
		...
		local ckHMAC = hmac.new('sha256',salted_password)
		local client_key = ckHMAC:final('Client Key', true)
		local stored_key = digest.digest('sha256', client_key, true)
		local auth_message = concat({client_first_message,server_first_message,client_final_message},',')
		local csHMAC = hmac.new('sha256', stored_key)
		...
		local skHMAC = hmac.new('sha256', salted_password)
		local server_key = skHMAC:final('Server Key', true)
		local ssHMAC = hmac.new('sha256', server_key)
